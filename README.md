...
* **`h1_description/`**: Содержит urdf-описание робота, launch-фалы, которые взаимодействуют с urdf-описанием.
*   **`h1_move_joint_rviz/`**: Содержит ноды, которая заставляет модель робота в rviz двигаться в соответствии с показаниями реальных или целевых положений суставов. (Для первого варианта нужен реальный робот, для второго - нет)
...
-   [unitree_h1_control_ws](https://github.com/cyberbanana777/unitree_h1_control_ws) - наша разработка. Репозиторий репозиторий, в котором содержится библиотека для удобной работы с роботом Unitree H1 и не только. Инструкция по установке находится внутри репозитория и также есть скрипт, позволяющий развернуть репозиторий быстро. Находится он [здесь](https://github.com/cyberbanana777/unitree_h1_meta_launch_ws?tab=readme-ov-file#соло-скрипты). Подробнее о возможностях репозиториев читайте в инструкциях, которые находится внутри репозиториев.
...
```
#####  Нода для визуализации реальных движений (нельзя без робота)
```bash
...
| Тип услуги | Топик                   | Тип сообщения                 | Описание                                                                                |
| :--------- | :---------------------- | :---------------------------- | :-------------------------------------------------------------------------------------- |
| Публикация | `/joint_states`         | `sensors_msgs/msg/JointState` | Угловые положения для суставов Unitree H1 в визуализации в rviz2                        |
| Подписка   | `/positions_to_unitree` | `std_msgs/msg/String`         | Целевые угловые положения для суставов Unitree H1 в допустимом диапазоне для Unitree H1 |
...
#### `robot_description_and_tfs_launch`
> Здесь будет картинка
#### `display_with_control_launch.py`
> Здесь будет картинка
#### `display_without_control_launch.py`
